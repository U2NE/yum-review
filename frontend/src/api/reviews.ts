...
export function getMyReviews() {
  return apiRequest<MyReview[]>('/api/me/reviews')
}
...
export function deleteReview(reviewId: number) {
  return apiRequest<void>(`/api/reviews/${reviewId}`, { method: 'DELETE' })
}
...
export function reviewErrorMessage(error: unknown) {
  ...
  if (error.status === 400) return '별점은 각 항목 1~5점으로 선택하고, 코멘트는 1,000자 이내로 작성해 주세요.'
  ...
  if (error.status === 409) return '이 메뉴에는 이미 리뷰가 있어요. 내 리뷰에서 수정해 주세요.'
