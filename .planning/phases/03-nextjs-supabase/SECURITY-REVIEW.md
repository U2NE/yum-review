...
| S4 | **잔여 위험 명시, 즉시 해제 보장은 미검증 · 보통** | 현재 소스 설정의 서명 URL TTL은 3,600초이고 업로드 객체 `cacheControl`도 3,600초다 (`lib/data/media.ts:4, 48`; `lib/data/catalog.ts:105, 241`; `lib/media/storage.ts:138, 242`). 보안 문서는 URL 만료와 브라우저 캐시를 구분하며, 캐시된 사본의 즉시 숨김을 보장하지 않는다 (`docs/security/media-access-window.md`). 실제 Storage 응답/CDN·브라우저 캐시 및 detach 직후 기존 URL 재요청은 로컬에서 검증하지 않았다. 새 서명 URL 발급을 막는 것과 이미 발급/캐시된 사본의 잔여 접근은 별개다. |
...
| S3·S4 이전 항목 | **이전 보고 유지.** 장소 검색은 Upstash/IP 제한과 fail-closed 코드를 갖지만 `x-real-ip`, Vercel/Upstash 설정 및 한도 동작은 검증하지 않았다. 사진 서명 URL TTL과 업로드 객체 `cacheControl`은 현재 모두 3,600초로 설정되어 있으며, 실제 Storage 응답/CDN·브라우저 캐시와 detach 후 기존 URL 재요청은 검증하지 않았다 (`docs/security/media-access-window.md`). | 기존 보안 검토의 해당 설정·캐시 조건을 disposable local/Preview에서 확인한다. |
...
## 2026-09-26 독립 보안 재검토 — 인증·역할·미디어 경계

**판정: 현재 코드에서 새 인증/역할 우회는 발견하지 못했으나, 전체 보안 승인과 운영 투입은 보류한다.** `proxy.ts`, SSR 클라이언트와 서버 가드, 관리자/업주 경로, 현재 Supabase 역할·Storage·미디어 검증 migrations를 정적으로 확인했다. 비밀 환경 파일, 호스팅 서비스, 데이터베이스 및 사용자 데이터를 조회하거나 수정하지 않았고, 이 검토 자체에서는 테스트를 실행하지 않았다.

### 정적 소스에서 확인한 경계

- `proxy.ts:16–61`은 세션 claim을 갱신하고, 기존 비밀번호 변경 대상은 강제 변경 화면으로 돌리며, 갱신 쿠키를 리디렉션에 복사한다. 쿠키를 공유 캐시에 내보내지 않도록 모든 proxy 응답에 `private, no-store`를 설정한다. 서버 가드는 별도로 `auth.getUser()`를 확인한 뒤 `get_my_access()`의 DB 역할을 사용한다 (`lib/auth/guards.ts:13–79`). 관리자와 업주 Server Action도 권한 가드를 다시 부른다 (`app/admin/page.tsx:14–24, 69–92`; 업주 화면은 `requireRestaurantOwner` 사용).
- 역할·정책 함수는 고정 `search_path`를 사용하고, private 스키마/테이블 접근은 회수한 뒤 필요한 함수만 명시적으로 허용한다. 업주 연결 RPC 안에서도 서버 관리자 권한을 확인한다. 테이블은 자기 리뷰·프로필·찜 같은 사용자 소유 조건과 관리자/해당 가게 업주 조건을 적용한다 (`20260925130001_rls_and_role_helpers.sql:528–545, 549–724, 923–1008`).
- 사진 버킷은 private이고, 대기 객체 업로드/읽기는 본인 UUID·현재 권한·`PENDING` 자산으로 제한된다. Storage UPDATE 정책은 없고, 활성 상태이며 메뉴/리뷰에 연결된 사진만 공개 읽기 정책을 통과한다 (`20260925130002_initial_storage_policies.sql:12–48`). 검증 API는 같은 출처·로그인 세션·자산 소유를 확인하고 caller JWT로만 Storage를 읽는다. 서버가 실제 저장 바이트와 디코딩 결과를 측정한 후 HMAC 증명을 만들며, SQL 활성화 함수가 proof/Vault 일치와 자산 상태를 재확인한다 (`app/api/media/verify/route.ts:55–120, 153–227`; `20260926170000_trusted_media_proofs_and_password_gate.sql:13–130`). 전역 검증 슬롯은 마지막 교체 migration에서도 `service_role`만 실행하도록 제한한다 (`20260926180002_media_and_review_integrity.sql:98–102, 234–238`).

### 기존 로컬 검증 기록과 이번 검토의 구분

`docs/migration/local-runtime-verification.md`에는 disposable 로컬 DB에 전체 12개 migrations를 적용하고, 합성 업주/관리자 권한과 일부 거부 사례, Storage 연결/분리 정책, `authenticated`의 서버 전용 슬롯 RPC 거부를 확인했다고 기록돼 있다. 이는 저장된 이전 로컬 실행 증거이며 이번 정적 검토에서 다시 실행하거나 DB 상태로 독립 확인한 결과가 아니다. 같은 기록은 Vault 키 양성 일치와 실제 이미지 검증 HTTP 경로는 실행하지 않았다고 명시한다. 역할/화면 57개 수용 행렬도 아직 완료되지 않았으므로, 이 문서만으로 전체 API 권한이나 브라우저 경계를 통과 처리할 수 없다.

### 열린 위험과 출시 전 증거

1. **보통 — 100MB 원본 제한은 일반 브라우저 클라이언트 기준이다.** 업로드 intent의 `original_bytes`는 호출자가 보내는 값이며 SQL이 저장하고 HMAC에 결속한다. 검증 서버는 Storage의 최종 객체 바이트를 독립 측정하지만 최적화 전 원본을 받거나 측정하지 않는다 (`20260925130001_rls_and_role_helpers.sql:810–870`; `app/api/media/verify/route.ts:97–113`; `docs/security/media-validation-key.md:22`). 변조 클라이언트는 실제 원본이 더 커도 낮은 크기를 신고할 수 있다. “100MB 미만”을 원본 파일에 대한 강제 보장으로 해석한다면 서버 측 수신/계측 경로가 필요하다.
2. **보통 — 연결 해제 후 즉시 사진 비공개를 보장하지 않는다.** 현재 소스의 서명 URL TTL은 3,600초이고 객체 업로드 `cacheControl`도 3,600초다 (`lib/data/media.ts:4, 49`; `lib/data/catalog.ts:105, 241`; `lib/media/storage.ts:138, 242`). `docs/security/media-access-window.md`의 기존 15초 기재는 정정되었지만, 실제 로컬 Storage 응답·CDN/브라우저 캐시와 detach 직후 기존 URL 재요청은 실행 검증되지 않았다. 정책상 새 일반 읽기 차단과 이미 발급/캐시된 URL·바이트의 잔여 접근은 별개이며, stale-cache 위험이 남는다. 로컬 동작을 검증하기 전에는 즉시 폐기로 표시하면 안 된다.
3. **보안 출시 gate — Vault 권한·키 일치 및 긍정 사진 검증은 미입증이다.** 로컬 기록은 키 부재 시 preflight가 `false`로 닫히는 것만 남긴다. migration 적용 DB의 Vault 읽기 권한, 키 일치, proof 활성화, 재사용 거부와 실제 이미지 HTTP 경로를 비밀값 노출 없이 disposable 환경에서 증명해야 한다. 호스팅 설정과 운영 환경은 이 검토에서 확인하지 않았다.

**최종 한계:** 위 평가는 정적 소스와 명시된 로컬 증거 문서의 분리된 요약이다. Hosted Supabase/Vercel의 실제 role grant, Auth 쿠키/메일 설정, Storage 서명 URL·캐시 동작, 운영 키 범위와 배포 한도는 검증하지 않았다. 따라서 정적 경계 개선은 확인했지만 QA·운영 보안 승인으로 승격하지 않는다.

## 2026-09-26 보완 — 서명 URL 및 캐시 수명 정정

기존의 15초 서명 URL 설명은 현재 소스와 일치하지 않아 정정한다. 단일/복수 이미지 URL 생성 경로는 모두 `SIGNED_READ_TTL_SECONDS = 60 * 60`을 사용하고 업로드 경로의 `cacheControl`도 3,600초다 (`lib/data/media.ts:4, 49`; `lib/data/catalog.ts:105, 241`; `lib/media/storage.ts:138, 242`). 위의 두 유효기간은 별개다. Supabase 공식 [Smart CDN 설명](https://supabase.com/docs/guides/storage/cdn/smart-cdn)은 캐시된 서명 응답이 토큰 만료보다 오래 제공될 수 있음을 설명한다. 공식 [서명 URL/다운로드 설명](https://supabase.com/docs/guides/storage/serving/downloads)은 URL을 만료 전까지 유효한 bearer 링크로 다루며 서명 URL 취소를 지원팀 절차로 안내한다.

**검토 판정:** 소스 설정을 읽은 정적 확인만 했다. 로컬 Storage의 실제 응답/캐시, 연결 해제 후 기존 URL 재요청, 브라우저 또는 CDN의 stale-cache 최장 시간을 실행 검증하지 않았다. 그러므로 정확한 유효 최대 시간은 아직 주장하지 않는다. `docs/security/media-access-window.md`를 새 설정과 이 한계에 맞춰 갱신했으며, 이 보완이 이전 15초 기재를 대체한다.
